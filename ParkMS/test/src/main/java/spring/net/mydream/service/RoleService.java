...
public class RoleService{
	...
	public List<SysRole> selectByUser() throws PreconditionException{
		
		if(true)
		{
			return DM.getSysRoleDao().findAll();
	...
	public List<SysMenu> getMenuByRole(Integer roleId) throws PreconditionException{
		SysRole role = (SysRole)GetData(DM.getSysRoleDao().findById(roleId));
		List<SysMenu> sms = new ArrayList<SysMenu>();
		sms = DM.getSysMenuDao().findByBelongedRole(role);
		
		if(StandardOPs.oclIsUndefined(sms).equals(false))
		{
			return sms;
